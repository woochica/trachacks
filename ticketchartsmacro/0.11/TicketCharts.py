...
def _create_javascript_array(array_name, values, function = lambda x : repr(str(x))):
...
def _get_stacked_bar_chart_stats(env, db, key, x_axis, query):
    ...
    # Returning all the keys is kind of an optimization (so that we won't look for all keys later on)
    keys = []
    ...
    # Stacked bar charts are represented by a dict of dicts : {'x_axis_value' : {'key_value' : number_of_tickets}}
    # Regular bar charts will be here the same, but 'key_value' will be replaced with None.
...
def _get_stacked_bar_max_y_value(ticket_stats):
    # x_axis_stats is a dict that its values are the number of tickets for each key
    return max([sum(x_axis_stats.itervalues()) for x_axis_stats in ticket_stats.itervalues()])
...
def _create_stacked_bar_on_click_html(env, key, x_axis, ticket_stats, query, function_name):
    """
...
""", {'array_code' : _create_javascript_array(array_name, ticket_stats.iterkeys()),
...
def _stacked_bars_graph(env, db, key, x_axis, query = None, title = None):
    ticket_stats, keys = _get_stacked_bar_chart_stats(env, db, key, x_axis, query)
    ...
    for i, key_value in enumerate(keys):
        ...
        if key:
            plot.append_keys(COLOURS[i], key_value, 13)  # colour, key-name, font-size
    ...
    return chart, chart_div_id, _create_stacked_bar_on_click_html(env, key, x_axis, ticket_stats, query, on_click_function_name)

def stacked_bars_graph(env, db, args):
    # Using **args here would be useful, but I want to be more precise
    return _stacked_bars_graph(env, db, key = args['key'], x_axis = args['x_axis'], query = args.get('query'),
                               title = args.get('title'))

def bars_graph(env, db, args):
    # I don't want code repetition, so we'll simply use stacked bars chart without keys.
    # This is kind of a hack, but it's better than copy-paste.
    return _stacked_bars_graph(env, db, key = None, x_axis = args['x_axis'], query = args.get('query'),
                               title = args.get('title'))

def _get_pie_graph_stats(env, db, factor, query = None):
    """
    Return a dict in which the keys are the factors and the values are the number of tickets of each factor.
    Example:
...
def _create_pie_graph_on_click_html(env, ticket_stats, factor, query, function_name):
    query_link = _get_query_link(env, query)
    array_name = 'array_%s' % (function_name, )
...
def create_graph(req, env, args):
    ...
    return additional_html + _get_chart_html(chart, chart_div_id, height = args['height'], width = args['width'],
                                             ofc_file = args['ofc_file'], js_dir = args['js_dir'],
...
def _get_chart_html(chart_object, chart_div_id, height = 300, width = 500, ofc_file = '/OFC.swf', js_dir = '/js', json_dir = '/js/json'):
...
"""
    return _safe_evaluate(chart_html, json_dir = json_dir, js_dir = js_dir, ofc_file = ofc_file,
                          chart_div_id = chart_div_id, width = width, height = height, chart_data = chart_object.encode(),
                          get_data_function = get_data_function)

def _set_base_path(req):
    # This is something better used as global, since it is a configuration dependent global.
    global BASE_PATH
