...
def existsPrivateComment(conn,ticket_id,comment_id):
	c = conn.cursor()
	try:
		c.execute('SELECT private FROM private_comment WHERE ticket_id=:ticket_id AND comment_id=:comment_id',
...
def createPrivateComment(conn,ticket_id,comment_id, private):
	c = conn.cursor()
	c.execute('INSERT INTO private_comment (ticket_id,comment_id,private) VALUES (:ticket_id,:comment_id,:private)',
