...
class PHPDocPlugin(Component):
    ...
    base_path = Option('phpdoc', 'path', '/var/cache/phpdoc/myproject/doc',
      """Directory containing phpdoc generated files.""")

    default_doc = Option('phpdoc', 'default_documentation', '',
      """TODO Default documentation project, relative to `[phpdoc] path`.
      When no explicit path is given in a documentation request,
      this path will be prepended to the request before looking
      for documentation files.""")

    html_output = Option('phpdoc', 'html_output', None,
      """TODO Default documentation project suffix, as generated by PHPdocumentor
      using the HTML_OUTPUT PHPDoc configuration setting.""")

    title = Option('phpdoc', 'title', 'PHPDoc',
      """Title to use for the main navigation tab.""")

    ext = Option('phpdoc', 'ext', 'htm html png',
      """Space separated list of extensions for phpdoc managed files.""")

    source_ext = Option('phpdoc', 'source_ext',
      'idl odl java cs py php php4 inc phtml m '
      'cpp cxx c hpp hxx h',
      """Space separated list of source files extensions""")

    index = Option('phpdoc', 'index', 'index.html',
      """Default index page to pick in the generated documentation.""")

    wiki_index = Option('phpdoc', 'wiki_index', None,
      """Wiki page to use as the default page for the PHPDoc main page.
      If set, supersedes the `[phpdoc] index` option.""")

    encoding = Option('phpdoc', 'encoding', 'iso-8859-1',
      """Default encoding used by the generated documentation files.""")

    default_namespace = Option('phpdoc', 'default_namespace', '',
    ...
    def get_navigation_items(self, req):
        if req.perm.has_permission('PHPDOC_VIEW'):
            # Return mainnav buttons.
            yield ('mainnav', 'phpdoc',
                   tag.a(self.title, href=req.href.phpdoc()))

    # IRequestHandler methods

    def match_request(self, req):
        if re.match(r'^/phpdoc(?:$|/)', req.path_info):
            if 'path' not in req.args: # not coming from a `phpdoc:` link
                segments = filter(None, req.path_info.split('/'))
                segments = segments[1:] # ditch 'phpdoc'
                if segments:
                    action, path, link = self._phpdoc_lookup(segments)
    ...
    def get_htdocs_dirs(self):
        ...
        return [('phpdoc', resource_filename(__name__, 'htdocs'))]
    ...
    def get_link_resolvers(self):
        def phpdoc_link(formatter, ns, params, label):
            if '/' not in params:
    ...
    def _phpdoc_lookup(self, segments):
        """Try to interpret path components as a request for phpdoc targets
        ...
        Return an `(action,path,link)` tuple, where:
         ...
         - `link` is the link to the resource, relative to the
           req.href.phpdoc base or a target in case of 'redirect'
        """
        ...
        if doc:
            ...
            if self.default_doc: # we can't stay at the 'phpdoc/' level
        ...
        #  - this doesn't work well with the CREATE_SUBDIRS PHPDoc option

        # do phpdoc-style name->file mapping
        # this is a little different than phpdoc, but I don't see another way
        # way to make phpdoc:Type<bool> links work, as it inserts a ' ' (or
