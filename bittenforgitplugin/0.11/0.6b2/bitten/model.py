...
class BuildConfig(object):
    ...
    _schema = [
        Table('bitten_config', key='name')[
            Column('name'), Column('branch'), Column('active', type='int'),
            Column('recipe'), Column('label'), Column('description')
        ]
    ]

    def __init__(self, env, name=None, branch=None, active=False, recipe=None,
    ...
    def insert(self, db=None):
        ...
        cursor = db.cursor()
        cursor.execute("INSERT INTO bitten_config (name,branch,active,"
                       "recipe,label,description) "
                       "VALUES (%s,%s,%s,%s,%s,%s)",
                       (self.name, self.branch, int(self.active or 0),
                        self.recipe or '',
    ...
    def update(self, db=None):
        ...
        cursor = db.cursor()
        cursor.execute("UPDATE bitten_config SET name=%s,branch=%s,active=%s,"
                       "recipe=%s,label=%s,"
    ...
    def fetch(cls, env, name, db=None):
        ...
        cursor = db.cursor()
        cursor.execute("SELECT branch,active,recipe,label,"
        ...
        config.branch = row[0] or ''
        config.active = bool(row[1])
        config.recipe = row[2] or ''
        config.label = row[3] or ''
        config.description = row[4] or ''
        return config
    ...
    def select(cls, env, include_inactive=False, db=None):
        ...
        cursor = db.cursor()
        if include_inactive:
            cursor.execute("SELECT name,branch,active,recipe,"
                           "label,description FROM bitten_config ORDER BY name")
        else:
            cursor.execute("SELECT name,branch,active,recipe,"
                           "label,description FROM bitten_config "
                           "WHERE active=1 ORDER BY name")
        for name, branch, active, recipe, label, description \
                in cursor:
            config = BuildConfig(env, name=name, branch=branch or '',
