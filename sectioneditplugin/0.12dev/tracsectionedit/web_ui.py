...
class WikiSectionEditModule(Component):
    ...
    def pre_process_request(self, req, handler):
        if re.match(r'^/($|wiki)', req.path_info) \
            ...
            if req.method=='POST' \
                ...
                fragment = '#' + req.args.get('anchor')
                req.redirect(req.href.wiki(req.args['page']) + fragment)
        return handler
    ...
    def filter_stream(self, req, method, filename, stream, data):
        if filename == 'wiki_edit.html' and 'section' in req.args and 'merge' not in req.args:
            ...
            anchor_element = html.input(type='hidden', name='anchor', id='anchor', value=''.join(req.args.get('anchor')))
            
            section_html = html(section_element, pre_element, post_element, anchor_element)
