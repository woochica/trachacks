...
""")
    ...
    if not page_releases: page_releases = [ 'stable' ]
...
""")
    ...
    if not write_tags(out, [x[0] for x in cursor.fetchall() if x[0] not in META_TAGS ], page_tags):
        out.write("<i>No tags available.</i>")
