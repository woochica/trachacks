...
function refreshFeed()
{
	feed.url = getReportUrl();
    if (!feed.url || feed.url.length < 1) {
        showMessageInContents(getLocalizedString("No Feed Specified"),
            getLocalizedString("Please specify a valid feed URL."));
        return false;
    }
...
function processFeedDocument(doc)
{
    ...
    if (doc) {
        ...
        if (results == null || results.length < 1) {
            showMessageInContents(getLocalizedString("No Items Found"),
        ...
        if (results == null || results.length < 1) {
            showMessageInContents(getLocalizedString("Nothing To Display"),
        ...
        // document is empty
        showMessageInContents(getLocalizedString("Invalid Feed"),
...
function load()
{
	...
	var tracURL = widget.preferenceForKey("tracURL");
    if(!tracURL)
		tracURL = attributes.feed.url;
	document.getElementById("tracURL").value=tracURL;
		
	var report = widget.preferenceForKey("report");
	if(report)
		document.getElementById("report").value=report;
		
	var user = widget.preferenceForKey("user");
	if(user)
...
function showFront(event)
{
	...
	widget.setPreferenceForKey(user.value,"user");

	var tracURL = document.getElementById('tracURL');
	widget.setPreferenceForKey(tracURL.value,"tracURL");
	refreshFeed();
...
function createNewTicket(event)
{
    // Insert Code Here
	 if (window.widget) {
		var tracURL = widget.preferenceForKey("tracURL");
        widget.openURL(tracURL + "/newticket");
        if (e) {
            e.stopPropagation();
            e.preventDefault();
        }
...
function changeReport(event)
{
    if(widget)
	{
		var selector = document.getElementById("report");
		widget.setPreferenceForKey(selector.value,"report");
		refreshFeed();
	}
}
...
function getReportUrl()
{
	...
	return tracURL.value + report.value + "&USER="+user.value;
}
...
function viewSource(event)
{
 // Insert Code Here
	 if (window.widget) {
		var tracURL = widget.preferenceForKey("tracURL");
        widget.openURL(tracURL + "/browser");
        if (e) {
            e.stopPropagation();
            e.preventDefault();
        }
...
function viewWiki(event)
{
 // Insert Code Here
	 if (window.widget) {
		var tracURL = widget.preferenceForKey("tracURL");
        widget.openURL(tracURL + "/wiki");
        if (e) {
            e.stopPropagation();
            e.preventDefault();
        }
...
function viewTimeline(event)
{
     // Insert Code Here
	 if (window.widget) {
		var tracURL = widget.preferenceForKey("tracURL");
        widget.openURL(tracURL + "/timeline");
        if (e) {
            e.stopPropagation();
            e.preventDefault();
        }
...
function viewRoadmap(event)
{
     // Insert Code Here
	 if (window.widget) {
		var tracURL = widget.preferenceForKey("tracURL");
        widget.openURL(tracURL + "/roadmap");
        if (e) {
            e.stopPropagation();
            e.preventDefault();
        }
