...
class TracAdmin(cmd.Cmd):
    ...
    def db_open(self):
        # self.env_open() 返回一个对象,对象中的特定方法可以打开实例数据库
        # 返回数据库对象
        return self.env_open().get_db_cnx()
    
    
    # 如果提供游标,就使用提供的游标查询,parems为query的参数
...
#               cls._help_config + cls._help_wiki +
    ...
    def do_initenv(self, line):
        ...
        'See also: TracAdmin, TracBacku, TracIni, TracGuide'],
        'TracFastCgi':
        ...
        'TracPlugins':
        [u'相关信息：[ZhTracGuide 帮助], [http://trac.edgewall.org/wiki/PluginList plugin list], [http://trac.edgewall.org/wiki/TracDev/ComponentArchitecture component architecture]',
        ...
        try:
            ...
            # 需要使用'/../'
            defaultDir = os.path.abspath(siteconfig.__default_templates_dir__ + '/../')
            self._do_wiki_load(defaultDir + '/zhwiki-default')
        ...
        try:
            ...
            # 因为有可以没有一些页面的中文版本
            help_zh = os.listdir(defaultDir + '/zhwiki-default')
            # \n 是后面安全去除添加内容的重要凭据
            headString = '\n\n[Zh%s 中文版本][[BR]]%s\n\n\n'
            headStringCN = '\n\n%s\n\n\n'
            for docName in help_zh:
                # 防止.svn目录
                if not os.path.isdir(defaultDir + '/zhwiki-default/' + docName):
                    print 'editing %s' % docName
                    # 取英文文件名
                    doc = docName[2:]
                    page = WikiPage(self.__env,doc)
                    if page.version == 0:
                        print '%s no exist!'
                        continue                    
                    ...
                    page.save('Trac','','')
                    # 中文文件
                    page = WikiPage(self.__env,docName)
                    if page.version == 0:
                        print '%s no exist!'
                        continue                    
                    seealso = seeAlso.get(doc,[''])
