  ...
  if:len(chrome.links.prev) ?>
   <li class="<?cs if:!len(chrome.links.next) ?> last<?cs /if ?>">
    &larr; <a href="<?cs var:chrome.links.prev.0.href ?>" title="<?cs
      var:chrome.links.prev.0.title ?>">较新版本</a>
   </li><?cs
  /if ?><?cs
  if:len(chrome.links.next) ?>
   <li class="last">
    <a href="<?cs var:chrome.links.next.0.href ?>" title="<?cs
      var:chrome.links.next.0.title ?>">较早版本</a> &rarr;
   </li><?cs
  /if ?>
 </ul>
</div>
...
<div id="content" class="log">
 ...
 <form id="prefs" action="<?cs var:browser_current_href ?>" method="get">
  <div>
   ...
   <label>显示从版本 <input type="text" id="rev" name="rev" value="<?cs
    var:log.items.0.rev ?>" size="5" /></label>
   <label>后退到版本 <input type="text" id="stop_rev" name="stop_rev" value="<?cs
    var:log.stop_rev ?>" size="5" />的日志</label>
   <br />
   <div class="choice">
    <fieldset>
     <legend>模式:</legend>
  ...
  <div id="legend">
   ...
   <dl>
    ...
    if:log.mode == "path_history" ?>
     <dt class="rem"></dt><dd>删除</dd><?cs
    /if ?>
    <dt class="mod"></dt><dd>修改</dd>
    <dt class="cp"></dt><dd>复制或重命名</dd>
   </dl>
  </div>
 </div>

 <form  class="printableform" action="<?cs var:log.changeset_href ?>" method="get">
  <div class="buttons"><input type="submit" value="显示修改" 
       title="显示旧版本与新版本的区别(从上面选择)" />
 </div>
 <table id="chglist" class="listing">
  <thead>
   <tr>
    ...
    <th class="change"></th>
    <th class="rev">版本</th>
    <th class="chgset">变更集</th>
    <th class="date">日期</th>
    <th class="author">作者</th>
    <th class="summary"><?cs if:!log.verbose ?>日志信息<?cs /if ?></th>
   </tr>
  </thead>
  <tbody><?cs
   set:indent = #1 ?><?cs
 ...
 if:len(links.prev) || len(links.next) ?><div id="paging" class="nav"><ul><?cs
  ...
  if:len(links.next) ?><li class="<?cs
   ...
   var:links.next.0.title ?>">旧版本</a> &rarr;</li><?cs
  /if ?></ul></div><?cs
 /if ?>

 <div id="help">
  <strong>提示:</strong> 查看 <a href="<?cs var:trac.href.wiki
  ?>/ZhTracRevisionLog">版本日志</a> 对本页面的帮助.
 </div>
