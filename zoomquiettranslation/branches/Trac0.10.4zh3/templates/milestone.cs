...
<?cs var:milestone.description_source ?></textarea></p>
   ...
   <div class="buttons">
    <?cs if:milestone.mode == "new"
     ?><input type="submit" value="增加里程碑" /><?cs
    else
     ?><input type="submit" value="提交" /><?cs
    /if ?>
 ...
 <?cs elif:milestone.mode == "delete" ?>
  <form action="<?cs var:milestone.href ?>" method="post">
   ...
   <div class="buttons">
    <input type="submit" name="cancel" value="取消" />
    <input type="submit" value="删除里程节点 " />
   </div>
  </form>
 <?cs else ?>
  ...
  <form id="stats" action="" method="get">
   <fieldset>
    <legend>
     ...
     /each ?></select>分类
     <noscript><input type="submit" value="刷新" /></noscript>
    </legend>
   ...
   <div class="buttons"><?cs
    if:trac.acl.MILESTONE_MODIFY ?>
     <form method="get" action=""><div>
      ...
      /if ?>
      <input type="submit" value="编辑里程信息 " accesskey="e" />
     </div></form><?cs
    /if ?><?cs
 ...
 <div id="help">
  <strong>提供:</strong> 查看 <a href="<?cs
    var:trac.href.wiki ?>/ZhTracRoadmap">项目路线</a> 对本页面的帮助.
 </div>
