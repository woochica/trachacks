...
class ReportModule(Component):
    ...
    def get_navigation_items(self, req):
        ...
        yield ('mainnav', 'tickets',
               html.A(u'传票检索 ', href=req.href.report()))
    ...
    def _render_view(self, req, db, id):
        ...
        """
        ZhRow = {u'Active Tickets':u'活动传票',
        u'Active Tickets by Version':u'活动传票(依版本)',
        u'Active Tickets by Milestone':u'活动传票(依里程碑)',
        u'Assigned, Active Tickets by Owner':u'已分配传票(依提交者排序)',
        u'Assigned, Active Tickets by Owner (Full Description)':u'已分配的活动传票(依创建者排序,全描述)',
        u'All Tickets By Milestone  (Including closed)':u'所有传票(依里程碑,含已关闭的)',
        u'My Tickets':u'我的传票',
        u'Active Tickets, Mine first':u'活动传票(我的为先)'}
        ZhDescribe = {'''
 ...
 * 先显示组内登录用户的所有传票。'''}
        ...
        try:
            ...
            ZhCols = {'Report':u'报表','Title':u'标题','Ticket':u'传票','Summary':u'描述','Component':u'组件','Version':u'版本','Milestone':u'里程碑','Type':u'类型','Owner':u'属主','Created':u'创建日期'}
        except Exception, e:
