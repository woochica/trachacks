...
<div id="ctxtnav" class="nav"><?cs
 with:links = chrome.links ?><?cs
  if:len(links.prev) || len(links.next) ?><ul><?cs
   if:len(links.prev) ?>
    <li class="first<?cs if:!len(links.up) && !len(links.next) ?> 最后<?cs /if ?>">
     &larr; <a href="<?cs var:links.prev.0.href ?>"><?cs
       var:links.prev.0.title ?></a>
    </li><?cs
   /if ?><?cs
   if:len(links.next) ?>
    <li class="<?cs if:!len(links.prev) && !len(links.up) ?>开始 <?cs /if ?>最后">
...
if:len(search.result) || len(search.quickjump) ?>
 ...
 <div id="notfound">没有匹配内容.</div><?cs
/if ?>

<div id="help">
 <strong>提示:</strong> 查看 <a href="<?cs
   var:trac.href.wiki ?>/ZhTracSearch">综合搜索</a>对本页面的帮助.
</div>
