...
<div id="content" class="wiki">
 ...
 <?cs if wiki.action == "delete" ?><?cs
  ...
  <form action="<?cs var:wiki.current_href ?>" method="post">
   ...
   <p><strong>您确定要把此页面<?cs
    if:!?wiki.version ?>版本<?cs
    /if ?>删除<?cs var:version_range ?>?</strong><br /><?cs
 ...
 <?cs elif:wiki.action == "diff" ?>
  <h1>变化: <?cs
    if:wiki.old_version ?>从 <a href="<?cs var:wiki.current_href ?>"><?cs var:wiki.page_name ?>的</a>
     <a href="<?cs var:wiki.current_href ?>?version=<?cs var:wiki.old_version?>">版本 <?cs var:wiki.old_version?></a> 和 <?cs
    else ?>从<?cs
    /if ?>
    <a href="<?cs var:wiki.current_href ?>?version=<?cs var:wiki.version?>">版本 <?cs var:wiki.version?></a>之间的变化
    </h1>
 ...
 <?cs else ?>
  <?cs if wiki.action == "edit" || wiki.action == "preview" || wiki.action == "collision" ?>
   <h1>正在编辑 "<?cs var:wiki.page_name ?>"</h1><?cs
    ...
    对不起，本页面在你编辑时已经有其它人同时修改了，所以你的修改不能被保存。
     </div><?cs
    /if ?>
...
<?cs var:wiki.page_source ?></textarea></p>
    ...
    <div id="help">
     <b>提示:</b>参考<a href="<?cs var:$trac.href.wiki
