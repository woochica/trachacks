  ...
  <table class="listing" id="dirlist">
   <thead>
    <tr><?cs 
     ...
     call:sortable_th(browser.order, browser.desc, 'date', '时间', browser.order_href) ?>
     <th class="change">最后修改时间</th>
    </tr>
   </thead>
   <tbody>
  ...
  <table id="info" summary="Revision info"><?cs
   if:!browser.is_dir ?><tr>
    <th scope="col">
     ...
     (检入者 :<?cs var:file.author ?>, <?cs var:file.age ?> 前)
    </th></tr><tr>
  ...
  <div id="anydiff">
   <form action="<?cs var:browser.anydiff_href ?>" method="get">
    <div class="buttons">
     ...
     <input type="submit" value="查看修改情况..." title="Prepare an Arbitrary Diff" />
    </div>
   </form>
  </div>
