   ...
   if:changeset.restricted ?>
    <h1><a title="Show full changeset" href="<?cs var:changeset.href.new_rev ?>">
     ...
     <a title="Show entry in browser" href="<?cs var:changeset.href.new_path ?>">
      <?cs var:changeset.new_path ?>版本<?cs var:changeset.new_rev ?>变更集</a> 
    </h1><?cs
   else ?>
    <h1>变更集 <?cs var:changeset.new_rev ?></h1><?cs
   /if ?><?cs
  else ?><?cs
    if:changeset.restricted ?>
    <h1>变更集: <a title="Show entry in browser" href="<?cs var:changeset.href.new_path ?>">
...
<form method="post" id="prefs" action="">
 <div><?cs 
  ...
  <div class="field">
   显示修改处上下<input type="text" name="contextlines" id="contextlines" size="2"
...
<dl id="overview"><?cs
 if:changeset.chgset ?>
 <dt class="property time">时间截:</dt>
 <dd class="time"><?cs var:changeset.time ?> 
  (<?cs alt:changeset.age ?>一个小时<?cs /alt ?> 前)</dd>
  ...
  <ul><?cs each:item = changeset.changes ?>
   <li><?cs
    if:item.change == 'add' ?><?cs
     ...
     call:node_change(item, 'rem', '删除') ?><?cs
    elif:item.change == 'copy' ?><?cs
     call:node_change(item, 'cp', '复制') ?><?cs
    elif:item.change == 'move' ?><?cs
     call:node_change(item, 'mv', '移动') ?><?cs
...
<div class="diff">
 <div id="legend">
  <h3>Legend:</h3>
  <dl>
   <dt class="unmod"></dt><dd>无修改</dd>
   <dt class="add"></dt><dd>增加</dd>
   <dt class="rem"></dt><dd>删除</dd>
   <dt class="mod"></dt><dd>修改</dd>
   <dt class="cp"></dt><dd>复制</dd>
   <dt class="mv"></dt><dd>移动</dd>
  </dl>
 </div>
 <ul class="entries"><?cs
