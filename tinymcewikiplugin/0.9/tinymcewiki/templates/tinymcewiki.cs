...
<?cs if wiki.action == "edit" || wiki.action == "preview" || wiki.action == "collision" ?><?cs if wiki.editor == "tinymce" ?><?cs include "tinymceconf.cs" ?><?cs /if ?><?cs /if ?>
   ...
   <div class="buttons"><?cs
    if:trac.acl.WIKI_MODIFY ?>
     <form method="get" action="<?cs var:wiki.current_href ?>"><div>
      ...
      <input type="hidden" name="editor" value="tinymce" />
      <input type="submit" value="<?cs if:wiki.exists ?>Edit<?cs
        else ?>Create<?cs /if ?> this page with WISIWYG" accesskey="e" />
     </div></form><?cs
     if:wiki.exists ?>
...
<?cs if wiki.action == "edit" || wiki.action == "preview" || wiki.action == "collision" ?><?cs if wiki.editor == "tinymce" ?>
<div>
<a href="http://tinymce.moxiecode.com?id=powered_by_wysiwyg"><img src="http://tinymce.sourceforge.net/buttons/powered_by_wysiwyg.png" border="0" width="88" height="32" alt="Powered by Moxiecode WYSIWYG" /></a>
</div>
<?cs /if ?><?cs /if ?>
