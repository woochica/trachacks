...
<div id="ctxtnav" class="nav"> 
	...
	<ul>
		<li class="first">
			...
			<?cs else ?><a href="<?cs var:trac.href.codeReview ?>">My Code Reviews</a>
			<?cs /if ?>
		</li>
		<li>
			<?cs if:create == "yes" ?>Create a Code Review
			<?cs else ?><a href="<?cs var:trac.href.newCodeReview ?>">Create a Code Review</a>
			<?cs /if ?>
		</li>
		...
		<li class="last">
			...
			<?cs else ?><a href="<?cs var:trac.href.options ?>">Manager Options</a>
			<?cs /if ?>
		</li>
		<?cs /if ?>
	</ul> 
        ...
        <?cs else ?>
            <?cs each:item = files ?>
                <tr class="<?cs if:name(item) % #2 ?>even<?cs else ?>odd<?cs /if ?>">
                    <td><a href="<?cs var:trac.href.performCodeReview?>?IDFile=<?cs var:item[0]?>"><?cs var:item[0]?></a></td>
                    <td><a href="<?cs var:trac.href.performCodeReview?>?IDFile=<?cs var:item[0]?>"><?cs var:item[2]?></a></td>
                    <td><a href="<?cs var:trac.href.performCodeReview?>?IDFile=<?cs var:item[0]?>"><?cs var:item[5]?></a></td>
                    <td><a href="<?cs var:trac.href.performCodeReview?>?IDFile=<?cs var:item[0]?>"><?cs var:item[3]?>-<?cs var:item[4]?></a></td>
                </tr>
            <?cs /each ?>
        <?cs /if ?>
    </tbody>
...
<table>
    <tr>
        <td>
            <form method="post" action="<?cs var:trac.href.viewCodeReview?>?Review=<?cs var:reviewID ?>&Vote=yes">
            <input type="submit" value="Approve">
            </form>
        </td>
        <td>
            <form method="post" action="<?cs var:trac.href.viewCodeReview?>?Review=<?cs var:reviewID ?>&Vote=no">
...
<table>
	<tr>
		<td>
			<form method="post" action="<?cs var:trac.href.viewCodeReview?>?Review=<?cs var:reviewID?>&Inclusion=1">
        ...
        <?cs if:status != "Closed" ?>
		<td>
			<form method="post" action="<?cs var:trac.href.viewCodeReview?>?Review=<?cs var:reviewID?>&Close=1">
        ...
        <td>
            <form method="post" action="">
                <select name="ManagerChoice" id="ManagerChoice">
                    ...
                    <option value="Ready for inclusion">Ready for inclusion
