...
<form method="post" action="<?cs var:trac.href.newCodeReview?>" onsubmit="return validateInput(this);">
Name: <input type="text" name="Name" value="<?cs var:name?>"><br>
...
// Callback variable
var browserCallback = "<?cs var:trac.href.codeReviewBrowser ?>";
...
function removeuser(txt) {
	...
	for (loop = 0; loop < table.rows.length; loop++) {
		...
		if (cell.getAttribute('value') == txt) {
...
//Ensure all fields are satisfactorily filled out before creating the CodeReview
...
function processBrowser(parent)
{
    ...
    if(parent.tagName == "A")
        {
            if(parent.href.indexOf("codeReviewBrowser") >= 0)
                {
...
function addButtonEnable()
{
	...
	if(document.getElementById('L' + end) == null)
		return;
