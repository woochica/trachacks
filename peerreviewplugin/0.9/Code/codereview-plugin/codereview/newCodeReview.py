...
class UserbaseModule(Component):
    ...
    def match_request(self, req):
        return req.path_info == '/newCodeReview'
    ...
    def process_request(self, req):
        ...
        if reviewID != None and (dbBack.getCodeReviewsByID(reviewID).Author == util.get_reporter_id(req) or req.perm.has_permission('CODE_REVIEW_MGR')):
            ...
            if req.args.get('reqAction') == 'createCodeReview':
                ...
                req.redirect(self.env.href.viewCodeReview() + '?Review=' + returnid)
            req.hdf['new'] = "yes"     
        req.hdf['users'] = allUsers
        req.hdf['trac.href.codeReviewBrowser'] = self.env.href.codeReviewBrowser()
	add_stylesheet(req, 'common/css/code.css')
        add_stylesheet(req, 'common/css/browser.css')
        return 'newCodeReview.cs', None
