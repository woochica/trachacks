...
# This is a modified version of the regular Trac browser, made so that
# in works in an AJAX callback framework.
...
class codeReviewBrowser(Component):
    ...
    def match_request(self, req):
        import re
        match = re.match(r'/(codeReviewBrowser|file)(?:(/.*))?', req.path_info)
        if match:
            req.args['path'] = match.group(2) or '/'
            if match.group(1) == 'file':
                # FIXME: This should be a permanent redirect
                req.redirect(self.env.href.codeReviewBrowser(req.args.get('path'),
    ...
    def process_request(self, req):
        ...
        req.hdf['browser'] = {
            ...
            'href': util.escape(self.env.href.codeReviewBrowser(path, rev=rev or
                                                      repos.youngest_rev)),
        ...
        return 'codeReviewBrowser.cs', None
    ...
    def get_path_links_CRB(self, href, path, rev):
        ...
        for part in parts:
            ...
            links.append({
                'name': part or 'root',
                'href': util.escape(href.codeReviewBrowser(path, rev=rev))
            })
        return links
    ...
    def _render_directory(self, req, repos, node, rev=None):
        ...
        for entry in node.get_entries():
            ...
            info.append({
                ...
                'browser_href': util.escape(self.env.href.codeReviewBrowser(entry.path,
                                                                  rev=rev))
            })
        ...
        if format in ['raw', 'txt']:
            ...
            if not is_binary(content):
                if mime_type != 'text/plain':
                    plain_href = self.env.href.codeReviewBrowser(node.path,
            ...
            raw_href = self.env.href.codeReviewBrowser(node.path, rev=rev and node.rev,
                                             format='raw')
    ...
    def _format_link(self, formatter, ns, path, label):
        ...
        return '<a class="source" href="%s%s">%s</a>' \
               % (util.escape(formatter.href.codeReviewBrowser(path, rev=rev)), anchor,
                  label)
