...
class UserbaseModule(Component):
    ...
    def match_request(self, req):
        return req.path_info == '/commentCallback'
    ...
    def process_request(self, req):
        if not (req.perm.has_permission('CODE_REVIEW_MGR') or req.perm.has_permission('CODE_REVIEW_DEV')):
            req.hdf['invalid'] = 4
            return 'commentCallback.cs', None
        ...
        if upload.filename:
            ...
            if size != 0:
                ...
                if sys.version_info[0] > 2 or (sys.version_info[0] == 2 and sys.version_info[1] >= 3):
                    self.filename = unicodedata.normalize('NFC',unicode(filename,'utf-8')).encode('utf-8')
                attachments_dir = os.path.join(os.path.normpath(self.env.path),'attachments')
                commonprefix = os.path.commonprefix([attachments_dir, self.path])
                assert commonprefix == attachments_dir
                ...
                try:
                    ...
                    struct.AttachmentPath = filename
                finally:
                    targetfile.close()
    ...
    def buildCommentHTML(self, comment, nodesIn, LineNum, IDFile, first):
        ...
        childrenHTML = ""
        for key in comment.Children.keys():
        ...
        if comment.AttachmentPath != "":
            html += "<a border=0 alt=\"Code Attachment\"  href=\"" + self.env.href.commentCallback() + "?actionType=getCommentFile&fileName=" + comment.AttachmentPath + "&IDFile=" + IDFile + "\"><img src=\"" + self.env.href.chrome() + "/hw/images/paper_clip.gif\"> " +  comment.AttachmentPath + "</a>"
        html += "</td>"
