...
<div id="ctxtnav" class="nav"><?cs
 if:query.report_href ?><ul>
  ...
  <li class="last">Requête personnalisée</li></ul><?cs
 /if ?>
</div>
...
<form id="query" method="post" action="<?cs var:trac.href.query ?>">
 <fieldset id="filters">
  ...
  /def ?>
  <table summary="Filtres de requête">
 ...
 <p class="option">
  <label for="group">Grouper les résultats par</label>
  ...
  <label for="groupdesc">décroissant</label>
 ...
 <p class="option">
  ...
  <label for="verbose">Afficher la description complète sous chaque résultat</label>
 </p>
 <div class="buttons">
 ...
 <thead><tr><?cs each:header = query.headers ?>
  <th class="<?cs var:header.name ?><?cs if:query.order == header.name ?> <?cs
   ...
   <a title="Trier par <?cs var:header.label ?><?cs
     if:query.order == header.name && !query.desc ?> (décroissant)<?cs
...
<?cs if:len(query.results) ?><?cs
 ...
 each:result = query.results ?><?cs
  ...
  <tr class="<?cs
   ...
   if:result.added ?> ajouté<?cs /if ?><?cs
   if:result.changed ?> changé<?cs /if ?><?cs
   if:result.removed ?> supprimé<?cs /if ?>"><?cs
  ...
  <?cs if:query.verbose ?>
    ...
    <p class="meta">Rapporté par <strong><?cs var:result.reporter ?></strong>,
...
<div id="help">
 <strong>Remarque :</strong> Veuillez consulter la page <a href="<?cs var:trac.href.wiki ?>/TracQuery">TracQuery</a> 
 pour plus d'informations sur l'utilisation des requêtes.
</div>
