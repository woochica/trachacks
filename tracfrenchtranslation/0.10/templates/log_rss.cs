...
<rss version="2.0">
 <channel><?cs 
  if:project.name_encoded ?>
   <title><?cs var:project.name_encoded ?>: Révisions de <?cs var:log.path ?></title><?cs 
  else ?>
   <title>Révisions de <?cs var:log.path ?></title><?cs 
  /if ?>
  ...
  each:item = log.items ?><?cs 
   with:change = log.changes[item.rev] ?>
    <item><?cs
     ...
     <title>R�vision <?cs var:item.rev ?>: <?cs var:change.shortlog ?></title>
