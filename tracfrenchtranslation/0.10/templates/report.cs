...
<div id="ctxtnav" class="nav">
 ...
 <ul><li class="first"><?cs
  ...
  if:report.query_href ?><li class="last"><a href="<?cs
   var:report.query_href ?>">Requête personnalisée</a></li><?cs
  /if ?></ul>
</div>
...
<div id="content" class="report">
  ...
  <h1><?cs var:title ?><?cs
    ...
    if:report.can_create || report.can_modify || report.can_delete ?>
     <div class="buttons"><?cs
      if:report.can_modify ?><form action="" method="get"><div>
       ...
       <input type="submit" value="Édition du rapport" accesskey="e" />
      </div></form><?cs /if ?><?cs
   ...
   if:report.id == -1 && report.can_create?><div class="buttons">
    <form action="" method="get"><div>
     ...
     <input type="submit" value="Créer un nouveau rapport" />
    </div></form></div><?cs
   /if ?><?cs
   if report.message ?>
    <div class="system-message"><?cs var report.message ?></div><?cs
   elif:idx == #0 ?>
    <div id="report-notfound">Aucun correspondance trouvée.</div><?cs
   /if ?>
 ...
 <?cs elif:report.mode == "delete" ?>
  ...
  <form action="<?cs var:report.href ?>" method="post">
   ...
   <input type="hidden" name="action" value="delete" />
   <p><strong>Etes-vous certain de vouloir détruire ce rapport?</strong></p>
   <div class="buttons">
