...
<div id="ctxtnav" class="nav">
 <ul>
  <li class="first <?cs if:len(chrome.links.prev)+len(chrome.links.next) == 0 ?>last<?cs /if ?>">
   <a href="<?cs var:log.browser_href ?>">Afficher la dernière révision</a>
  </li><?cs
...
<div id="content" class="log">
 ...
 <form id="prefs" action="<?cs var:browser_current_href ?>" method="get">
  <div>
   ...
   <label>Afficher l'historique du dépôt à partir de <input type="text" id="rev" name="rev" value="<?cs
  ...
  <div class="buttons">
   <input type="submit" value="Actualiser" 
          title="Remarque : En actualisant, l'historique de la page est remis à zero" />
  </div>
 </form>
   ...
   each:item = log.items ?><?cs 
    ...
    <tr class="<?cs var:even_odd ?>">
     ...
     <td class="change" style="padding-left:<?cs var:indent ?>em">
      <a title="Afficher l'historique en commencant à partir de la révision" href="<?cs var:item.log_href ?>">
 ...
 <div id="help">
  ...
  ?>/TracRevisionLog">TracRevisionLog</a> pour plus d'informations sur l'utilisation de l'historique des révisions.
 </div>
