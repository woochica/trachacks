...
class NoSuchChangeset(TracError):
    def __init__(self, rev):
        TracError.__init__(self, u"Aucune version %s n'est répertoriée dans le dépôt" \
                                 % rev)
    ...
    def assert_permission(self, path):
        if not self.has_permission(path):
            raise PermissionDenied, \
                  u"Droits d'accès insuffisants pour accèder à %s" % path

    def assert_permission_for_changeset(self, rev):
        if not self.has_permission_for_changeset(rev):
            raise PermissionDenied, \
                  u"Droits d'accès insuffisants pour accèder à la version %s" % rev
