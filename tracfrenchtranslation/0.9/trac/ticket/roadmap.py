...
class RoadmapModule(Component):
    ...
    def get_navigation_items(self, req):
        if not req.perm.has_permission('ROADMAP_VIEW'):
            return
        yield 'mainnav', 'roadmap', '<a href="%s" accesskey="3">Feuille ' \
                         'de route</a>' % self.env.href.roadmap()
...
class MilestoneModule(Component):
    ...
    def get_timeline_events(self, req, start, stop, filters):
        if 'milestone' in filters:
            ...
            for completed,name,description in cursor:
                title = 'Jalon <em>%s</em> completé' % escape(name)
                if format == 'rss':
    ...
    def _do_save(self, req, db, milestone):
        ...
        if not req.args.has_key('name'):
            raise TracError('Vous devez indiquer un nom pour le jalon.',
        ...
        try:
            ...
            raise TracError(e, 'Format de date invalide')
        if req.args.has_key('completed'):
            ...
            if milestone.completed > time():
                raise TracError('La date de livraison ne peut être dans le futur',
