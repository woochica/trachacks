...
""")
    ...
    def _do_create(self, req, db):
        if not req.args.get('summary'):
            raise TracError('Les tickets doivent contenir un intitulé.')
...
class TicketModule(Component):
    ...
    def process_request(self, req):
        ...
        if 'query_tickets' in req.session:
            ...
            if str(id) in tickets:
                ...
                if idx > 0:
                    add_link(req, 'premier', self.env.href.ticket(tickets[0]),
                             'Ticket #%s' % tickets[0])
                    add_link(req, 'précédent', self.env.href.ticket(tickets[idx - 1]),
                             'Ticket #%s' % tickets[idx - 1])
                if idx < len(tickets) - 1:
                    add_link(req, 'suivant', self.env.href.ticket(tickets[idx + 1]),
                             'Ticket #%s' % tickets[idx + 1])
                    add_link(req, 'dernier', self.env.href.ticket(tickets[-1]),
                             'Ticket #%s' % tickets[-1])
        ...
        if 'ticket' in filters:
            ...
            for t, id, resolution, status, type, message, author, summary \
                ...
                title = 'Ticket <em title="%s">#%s</em> (%s) %s par %s' % (
                        util.escape(summary), id, 
                        util.translate(self.env, type),
                        util.translate(self.env, verbs[status]), 
                        util.escape(author))
                if format == 'rss':
    ...
    def _do_save(self, req, db, ticket):
        ...
        if int(req.args.get('ts')) != ticket.time_changed:
            raise TracError("Désolé, impossible d'enregistrer vos modifications. "
                            "Ce ticket a été modifié par quelqu'un d'autre "
                            "depuis que vous avez commencé", 'Collision en plein vol')

        # Do any action on the ticket?
        action = req.args.get('action')
        actions = TicketSystem(self.env).get_available_actions(ticket, req.perm)
        if action not in actions:
            raise TracError('Action invalide')
        ...
        try:
            ...
            self.log.exception("Echec d'envoi de la notification de modification du "
                               "ticket #%s: %s" % (ticket.id, e))
...
class UpdateDetailsForTimeline(Component):
    ...
    def get_timeline_filters(self, req):
        if self.config.get('timeline', 'ticket_show_details') in util.FALSE:
            return
        if req.perm.has_permission('TICKET_VIEW'):
            yield ('ticket_details', 'Détail ticket', False)
        ...
        if 'ticket_details' in filters:
            ...
            for time,id,type,field,oldvalue,newvalue,author,summary in cursor:
                ...
                if field == 'comment':
                    ...
                    field_changes.append(util.translate(self.env, field))
            if previous_update and not ticket_change:
                updates.append((previous_update,field_changes,comment))
            ...
            for (t,id,author,type,summary),field_changes,comment in updates:
                if absurls:
                    ...
                    href = self.env.href.ticket(id) 
                title = 'Ticket <em title="%s">#%s</em> (%s) mis à jour par %s' \
                        % (util.escape(summary), id, 
                           util.escape(util.translate(self.env, type)), 
                           util.escape(author))
                message = ''
                if len(field_changes) > 0:
                    message = ', '.join(field_changes) + ' modifié'
