...
class RequestDispatcher(Component):
    ...
    def dispatch(self, req):
        ...
        if not chosen_handler:
            ...
            raise TracError, 'Aucun gestionnaire défini pour la requête %s' % req.path_info
...
def populate_hdf(hdf, env, req=None):
    ...
    hdf['project'] = {
        ...
        'footer': env.config.get('project', 'footer',
                 'Consultez le projet open source Trac sur<br />'
...
def send_pretty_error(e, env, req=None):
    ...
    try:
        ...
        if isinstance(e, TracError):
            req.hdf['title'] = e.title or 'Erreur'
            req.hdf['error.title'] = e.title or 'Erreur'
            ...
            req.hdf['title'] = 'Permission refusée'
...
def get_environment(req, options, threaded=True):
    if 'TRAC_ENV' in options:
        ...
        raise TracError, \
              "Les variables d'environnement 'TRAC_ENV' ou 'TRAC_ENV_PARENT_DIR'\n" \
              "ou bien les options mod_python 'TracEnv' or 'TracEnvParentDir'\n" \
              "ne sont pas définies. Trac a besoin d'une de ces options pour\n" \
              "pouvoir localiser le ou les environnement(s) Trac."
