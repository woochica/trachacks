...
class DirAuthStore(Component):
    ...
    cache_memsize_warn = Option('account-manager', 'cache_memsize_warn', 300,
    ...
    def get_users(self, populate_session=True):
        ...
        # -- check memcache
        user_list = self._cache_get('allusers')
        if user_list:
            return user_list
        ...
        self._cache_set('allusers', user_list)
    ...
    def check_password(self, user, password):
        ...
        if self.group_validusers:
            user_groups = self._expand_user_groups(user, NOCACHE)
            if not self.group_validusers in user_groups:
    ...
    def _populate_user_session(self, userinfo):
        ...
        try:
            cur.execute(
                "INSERT INTO session_attribute (sid, authenticated, name, value) VALUES ('%s', 1, 'enabled', '1')"
                % uname)
        except:
            self.log.debug('session for %s exists.' % uname)
    ...
    def _dir_search(self, basedn=None, scope=1, lfilter=None, attrs=[], check_cache=1):
        ...
        d = self._bind_dir()
        self.log.debug(
            '_dir_search: starting LDAP search of %s %s using %s for %s ' % (self.dir_uri, basedn, lfilter, attrs))
