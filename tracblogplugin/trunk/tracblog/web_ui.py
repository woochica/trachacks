...
class TracBlogPlugin(WikiMacroBase):
...
#               IWikiMacroProvider, IPermissionRequestor)
    ...
    nav_bar = BoolOption('blog', 'nav_bar', True, doc="True or False.  Whether to show "
                         "the link to the blag handler in the main navigation bar.")
    nav_bar_text = Option('blog', 'nav_bar_text', 'Blog',  doc="The text to show in the "
                          "navigation bar entry linking to the blog handler.")
    ...
    def get_permission_actions(sef):
        return ['BLOG_VIEW']
    ...
    def get_navigation_items(self, req):
        if req.perm.has_permission('BLOG_VIEW') and self.nav_bar:
            yield 'mainnav', 'blog', tag.a(self.nav_bar_text, href= req.href.blog())
    ...
    def expand_macro(self, formatter, name, content):
        ...
        if not tags:
            tags = [t.strip() for t in self.default_tag]
