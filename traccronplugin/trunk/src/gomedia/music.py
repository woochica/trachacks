...
class MusicListCreator(RandomListCreator):
    ...
    def __init__(self, size_limit=None, global_size_limit=None, _directory=None, _destination=None):
        ...
        self.VALID_TYPE = ['.MP3','.OGG','.OGA','.WAV']
...
def run():
    ...
    parser.add_option("-d", "--directory", action="store", type="string", dest="directory", help="root dircetory to scan for media files")
    parser.add_option("-o", "--output", action="store", type="string", dest="output", help="file to write the random file set to")    
    options, args = parser.parse_args()
    MusicListCreator(options.max_file_size, options.max_global_size, options.directory, options.output).create_random_list()
