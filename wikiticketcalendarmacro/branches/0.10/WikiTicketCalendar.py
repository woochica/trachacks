...
# think this stuff is worth it, you can buy me a beer in return.  Matthew Good
...
# Display milestone name with accents, fix provided as anonymous contribution.
...
def execute(hdf, txt, env):
    ...
    showbuttons = 1
    if len(args) >= 3:
        showbuttons = bool(args[2]=="True" or args[2]=="true" or
                           args[2]=="no" or args[2]=="0")
    ...
    for row in cal:
        ...
        for day in row:
            if not day:
                ...
                # (date as single numbers -> string -> time string parser
                # time string parser -> struct_time -> mktime -> seconds)
                ...
                buff.write("""<td%(style)s width="110"
                    valign="top"><b>%(day)s</b>
                    """ % {
                    'day': day,
                    'style': day == curr_day and
                        """ style="background: #fbfbfb; border-color: #444444;
                        color: #444; border-style:solid; border-width:1px;"
                        """ or
                        """ style="background: #e5e5e5; border-color: #444444;
                        color: #333; border-style:solid; border-width:1px;"
                        """,
                    })
                ...
                while (1):
                    ...
                    if row == None:
                        ...
                        buff.write("""
                            ...
                            width="110"><tr><td style="font-size: 9px;
                            background: #f7f7f0; border: 1px solid #d7d7d7;
                            border-bottom-color: #999;" align="left"><a
                            href="%(url)s">* %(name)s</a></td></tr></table>
                            """ % {
                            'name': name,
                            'url': url,
                            })
                ...
                cursor.execute("""
                    SELECT t.id,t.summary,t.keywords||' - '||t.owner,
                ...
                while (1):
                    ...
                    if row == None:
                        ...
                        description = "" #row[4]
                        buff.write("""
                            <div style="%(style)s" align="left">%(ticket)s (<a
                            href="%(url)s" title=\'%(description)s\'
                            target="_blank">%(owner)s</a>)</div><p>
                            """ % {
                            'url': url,
                            'day': day,
                            'ticket': ticket[0:100],
                            'owner': owner,
                            'style': status == 'closed' and
