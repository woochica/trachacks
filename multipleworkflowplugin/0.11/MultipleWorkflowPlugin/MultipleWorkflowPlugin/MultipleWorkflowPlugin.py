...
def parse_workflow_config(rawactions):
    """Given a list of options from [ticket-workflow-xxxxx] create the inner workflow model and normalize it's values"""
    actions = {}
...
class MultipleWorkflowPlugin(Component):
    """Ticket action controller which provides actions according to a workflow defined in the TracIni configuration file, inside the
    [ticket-workflow-type] session.It manages multiple workflows base on ticket type.If a session doesn't exist 
    it returns the default workflow session[ticket-workflow]
    """
...
"""
    ...
    # Public methods (for other ITicketActionControllers that want to use our config file and provide an operation for an action)
