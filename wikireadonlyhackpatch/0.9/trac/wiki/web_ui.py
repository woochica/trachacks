...
##########
        is_wiki_author = False
        cursor = db.cursor()
        #cursor.execute("SELECT * FROM wiki WHERE author=%s AND name=%s ORDER BY version ASC LIMIT 1", (req.authname,page.name) )
        cursor.execute("SELECT * FROM wiki WHERE author=%s AND name=%s AND version=1", (req.authname,page.name) )
        if cursor.fetchone():
            is_wiki_author = True
##########
        if page.readonly and not is_wiki_author:
            req.perm.assert_permission('WIKI_ADMIN')
