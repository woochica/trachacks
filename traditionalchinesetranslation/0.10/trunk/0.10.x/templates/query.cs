...
<div id="ctxtnav" class="nav"><?cs
 if:query.report_href ?><ul>
  <li class="first"><a href="<?cs
    var:query.report_href ?>">有效報表</a></li>
  <li class="last">自定查詢</li></ul><?cs
 /if ?>
</div>

<?cs def:num_matches(v) ?><span class="numrows">(<?cs 
 alt:v ?>沒有<?cs /alt ?> 符合的項目)</span><?cs
/def ?>
...
<form id="query" method="post" action="<?cs var:trac.href.query ?>">
  ...
  <table summary="Query filters">
   ...
   <tbody><tr class="actions">
    <td class="actions" colspan="4" style="text-align: right">
     <label for="add_filter">新增篩選條件</label>&nbsp;
 ...
 <p class="option">
  ...
  <label for="verbose">顯示完整描述</label>
 </p>
 <div class="buttons">
...
<div id="help">
 <strong>提示:</strong> 查看 <a href="<?cs var:trac.href.wiki ?>/ZhTracQuery">查詢</a> 
 線上手冊.
</div>
