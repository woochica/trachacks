...
<div id="ticket">
 <div class="date">
  <p title="<?cs var:ticket.opened ?>">新增時間 <?cs var:ticket.opened_delta ?> 前</p><?cs
  if:ticket.lastmod ?>
   <p title="<?cs var:ticket.lastmod ?>">更新時間 <?cs var:ticket.lastmod_delta ?> 前</p>
  <?cs /if ?>
 </div>
 <h2 class="summary"><?cs var:ticket.summary ?></h2>
 <table class="properties">
  <tr>
   <th id="h_reporter">新增:</th>
   <td headers="h_reporter"><?cs var:ticket.reporter ?></td>
   <th id="h_owner">指派給:</th>
   ...
   <div class="description">
    <h3 id="comment:description"><?cs
     ...
     <span class="inlinebuttons">
      ...
      <input type="submit" value="回復" title="Reply, quoting this description" />
     </span><?cs
     /if ?>
     描述<?cs
...
<?cs if:len(ticket.changes) ?><h2>修改歷程</h2>
<div id="changelog"><?cs
 ...
 <div class="change">
  <h3 <?cs if:change.cnum ?>id="comment:<?cs var:change.cnum ?>"<?cs /if ?>><?cs
   ...
   /if ?><?cs
   var:change.date ?> 編輯者 <?cs var:change.author ?>
  </h3><?cs
  if:len(change.fields) ?>
   <ul class="changes"><?cs
   each:field = change.fields ?>
    <li><strong><?cs var:name(field) ?></strong> <?cs
    if:name(field) == 'attachment' ?><em><?cs var:field.new ?></em>新增<?cs
    elif:field.old && field.new ?>從 <em><?cs
     var:field.old ?></em> 修改為 <em><?cs var:field.new ?></em><?cs
    elif:!field.old && field.new ?>設置為 <em><?cs var:field.new ?></em><?cs
    elif:field.old && !field.new ?>刪除<?cs
    else ?>修改<?cs
    /if ?>.</li>
    <?cs
   /each ?>
...
<form action="<?cs var:ticket.href ?>#preview" method="post">
 <hr />
 <h3><a name="edit" onfocus="document.getElementById('comment').focus()">新增/修改 #<?cs
   var:ticket.id ?> (<?cs var:ticket.summary ?>)</a></h3>
 <?cs if:trac.authname == "anonymous" ?>
  <div class="field">
   <label for="author">輸入使用者名稱或者信箱地址:</label><br />
 ...
 <div class="field">
  <fieldset class="iefix">
   <label for="comment">評論 (<a tabindex="42" href="<?cs
     var:trac.href.wiki ?>/ZhWikiFormatting">維基格式說明</a>幫助):</label><br />
 ...
 <?cs if:trac.acl.TICKET_CHGPROP ?><fieldset id="properties">
  ...
  <table><tr>
   ...
   if:trac.acl.TICKET_ADMIN ?><tr>
    <th><label for="description">詳細描述:</label></th>
...
<?cs var:ticket[name(field)] ?></textarea><?cs
 ...
 <fieldset id="action">
  ...
  if:ticket.actions.reassign ?><?cs
   ...
   <label for="reassign">重新指派給:</label>
   <label><?cs
