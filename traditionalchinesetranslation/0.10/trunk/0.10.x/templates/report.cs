...
<div id="ctxtnav" class="nav">
 ...
 <ul><li class="first"><?cs
   if:chrome.links.up.0.href ?><a href="<?cs
    var:chrome.links.up.0.href ?>">現有報表</a><?cs
   else ?>現有報表<?cs
  /if ?></li><?cs
  if:report.query_href ?><li class="last"><a href="<?cs
   var:report.query_href ?>">自定查詢</a></li><?cs
  /if ?></ul>
</div>
  ...
  <h1><?cs var:title ?><?cs
   ...
   if:report.id == -1 && report.can_create?><div class="buttons">
    <form action="" method="get"><div>
     ...
     <input type="submit" value="新增報表" />
    </div></form></div><?cs
   /if ?><?cs
   if report.message ?>
    <div class="system-message"><?cs var report.message ?></div><?cs
   elif:idx == #0 ?>
    <div id="report-notfound">沒有符合的項目.</div><?cs
   /if ?>
    ...
    <div>
     ...
     <div class="field">
      <label for="description">
       敘述:</label> (You may use <a tabindex="42" href="<?cs
...
<?cs var:report.description ?></textarea>
 ...
 <div id="help">
  <strong>提示:</strong> 查看 <a href="<?cs
    var:trac.href.wiki ?>/ZhTracReports">報表</a> 線上手冊
 </div>
