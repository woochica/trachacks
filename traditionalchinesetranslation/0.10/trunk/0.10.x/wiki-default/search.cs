...
<div id="ctxtnav" class="nav"><?cs
 with:links = chrome.links ?><?cs
  if:len(links.prev) || len(links.next) ?><ul><?cs
   if:len(links.prev) ?>
    <li class="first<?cs if:!len(links.up) && !len(links.next) ?> 最後<?cs /if ?>">
     &larr; <a href="<?cs var:links.prev.0.href ?>"><?cs
       var:links.prev.0.title ?></a>
    </li><?cs
   /if ?><?cs
   if:len(links.next) ?>
    <li class="<?cs if:!len(links.prev) && !len(links.up) ?>開始 <?cs /if ?>最後">
...
if:len(search.result) || len(search.quickjump) ?>
 ...
 <div id="notfound">沒有找到符合條件的內容.</div><?cs
/if ?>

<div id="help">
 <strong>提示:</strong> 查看 <a href="<?cs
   var:trac.href.wiki ?>/ZhTracSearch">進階搜尋</a>對本頁面的幫助.
</div>
