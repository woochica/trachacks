...
<div id="content" class="browser">
 ...
 <?cs if:browser.is_dir ?>
  <table class="listing" id="dirlist">
   <thead>
    <tr><?cs 
     call:sortable_th(browser.order, browser.desc, 'name', '文件名', browser.order_href) ?><?cs 
     call:sortable_th(browser.order, browser.desc, 'size', '大小', browser.order_href) ?>
     <th class="rev">版本</th><?cs 
     call:sortable_th(browser.order, browser.desc, 'date', '時間', browser.order_href) ?>
     <th class="change">最後修改時間</th>
    </tr>
   </thead>
   <tbody>
 ...
 if:len(browser.props) || !browser.is_dir ?>
  <table id="info" summary="Revision info"><?cs
   if:!browser.is_dir ?><tr>
    <th scope="col">
     版本 <a href="<?cs var:file.changeset_href ?>"><?cs var:file.rev ?></a>, <?cs var:file.size ?>
     (作者 :<?cs var:file.author ?>, <?cs var:file.age ?> 前)
    </th></tr><tr>
 ...
 <div id="help">
  <strong>Note:</strong> 查看<a href="<?cs var:trac.href.wiki
  ?>/ZhTracBrowser">瀏覽wiki碼</a>對本頁面的幫助
 </div>
  ...
  <div id="anydiff">
   <form action="<?cs var:browser.anydiff_href ?>" method="get">
    <div class="buttons">
     ...
     <input type="submit" value="查看修改情況..." title="Prepare an Arbitrary Diff" />
    </div>
   </form>
  </div>
