...
<div id="content" class="milestone">
 ...
 <?cs else ?>
 <h1>里程碑 <?cs var:milestone.name ?></h1>
 <?cs /if ?>
  ...
  <form id="edit" action="<?cs var:milestone.href ?>" method="post">
   ...
   <fieldset>
    ...
    <label>到期日:<br />
     ...
     <em>日期格式: <?cs var:milestone.date_hint ?></em>
    </label>
    <div class="field">
     <label>
     ...
     <label>
      ...
      <em>日期格式: <?cs var:milestone.datetime_hint ?></em>
     </label><?cs
     if:len(milestones) ?>
     <br/>
...
<?cs var:milestone.description_source ?></textarea></p>
   ...
   <div class="buttons">
    <?cs if:milestone.mode == "new"
     ?><input type="submit" value="新增里程碑" /><?cs
    else
     ?><input type="submit" value="送出" /><?cs
    /if ?>
   ...
   <div class="buttons"><?cs
    if:trac.acl.MILESTONE_MODIFY ?>
     <form method="get" action=""><div>
      ...
      /if ?>
      <input type="submit" value="編輯里程訊息 " accesskey="e" />
     </div></form><?cs
    /if ?><?cs
    if:trac.acl.MILESTONE_DELETE ?>
     <form method="get" action=""><div>
      <input type="hidden" name="action" value="刪除" /><?cs
      if:milestone.id_param ?>
       <input type="hidden" name="id" value="<?cs var:milestone.name ?>" /><?cs
      /if ?>
      <input type="submit" value="刪除" />
 ...
 <div id="help">
  <strong>提供:</strong> 查看 <a href="<?cs
    var:trac.href.wiki ?>/ZhTracRoadmap">項目路線</a> 線上手冊.
 </div>
