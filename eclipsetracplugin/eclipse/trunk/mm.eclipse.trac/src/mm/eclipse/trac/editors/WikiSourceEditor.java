...
public class WikiSourceEditor extends AbstractDecoratedTextEditor
{
    ...
    {
        super.createActions();
        IAction action = new ContentAssistAction( Activator.getDefault()
                .getResourceBundle(), "ContentAssistProposal.", this ); 
        action
                .setActionDefinitionId( ITextEditorActionDefinitionIds.CONTENT_ASSIST_PROPOSALS );
        setAction( WikiEditorContributor.CONTENTASSIST_ACTION, action ); 
        markAsStateDependentAction( WikiEditorContributor.CONTENTASSIST_ACTION, true ); 
    }
    ...
    {
        WikiPage page = (WikiPage) getEditorInput().getAdapter( WikiPage.class );
        String content = getDocumentProvider().getDocument( getEditorInput() )
                .get();
        try
        {
            page.putContent( content );
        } catch ( Throwable t )
        {
        }
    ...
    public void setSection( WikiSection section )
    {
        ...
        if ( contentOutline != null ) contentOutline.setWiki( section );
