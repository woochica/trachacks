...
public class WeakCollection<T> implements Collection<T>
{
    ...
    {
        Collection<WeakReference<T>> list = new ArrayList<WeakReference<T>>( c.size() );
        for ( T element : c )
    ...
    private int indexOf( Object item )
    {
        ...
        if ( item == null )
        {
            for ( int i = 0; i < size; i++ )
                if ( data.get( i ).get() == null ) return i;
        }
        else
        {
            for ( int i = 0; i < size; i++ )
                if ( item.equals( data.get( i ).get() ) ) return i;
        }
        return -1;
    }
    ...
    private class WeakIterator implements Iterator<T>
    {
        ...
        public void remove()
        {
            if ( cursor < 0 || cursor >= data.size() ) throw new IllegalStateException();
