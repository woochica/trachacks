...
<div id="ctxtnav" class="nav">
 ...
 <ul><li class="first"><?cs
   if:chrome.links.up.0.href ?><a href="<?cs
    var:chrome.links.up.0.href ?>">Dostêpne raporty</a><?cs
   else ?>Doostêpne raporty<?cs
  /if ?></li><?cs
...
<div id="content" class="report">
  ...
  <h1><?cs var:title ?><?cs
    ...
    if:report.can_create || report.can_modify || report.can_delete ?>
     <div class="buttons"><?cs
      ...
      if:report.can_delete ?><form action="" method="get"><div>
       ...
       <input type="submit" value="Usuñ raport" />
      </div></form><?cs /if ?>
     </div><?cs
    /if ?><?cs
   /if ?>
   ...
   if:report.id == -1 && report.can_create?><div class="buttons">
    <form action="" method="get"><div>
     ...
     <input type="submit" value="Stwórz nowy raport" />
    </div></form></div><?cs
   /if ?><?cs
   if report.message ?>
 ...
 <?cs elif:report.mode == "delete" ?>
  ...
  <form action="<?cs var:report.href ?>" method="post">
   ...
   <input type="hidden" name="action" value="delete" />
   <p><strong>Czy jeste¶ pewny ¿e chcesz skasowaæ ten raport?</strong></p>
   <div class="buttons">
    ...
    <div>
     ...
     <div class="field">
      <label for="description">
       Opis:</label> (mo¿esz u¿ywaæ tutaj <a tabindex="42" href="<?cs
         var:$trac.href.wiki ?>/WikiFormatting">formatowañ Wiki</a>)
      </label><br />
 ...
 <div id="help">
  <strong>Pomoc:</strong> Zobacz <a href="<?cs
    var:trac.href.wiki ?>/TracReports">TracReports</a>, aby dowiedzieæ siê wiêcej na temat tworzenia raportów.
 </div>
