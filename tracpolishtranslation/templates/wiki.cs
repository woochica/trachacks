...
<div id="ctxtnav" class="nav">
 <h2>Nawigacja</h2>
 <ul>
  <li><a href="<?cs var:trac.href.wiki ?>">Pocz±tek</a></li>
  <li><a href="<?cs var:trac.href.wiki ?>/TitleIndex">Index</a></li>
  <li><a href="<?cs var:trac.href.wiki ?>/RecentChanges">Ostatnie zmiany</a></li>
  <?cs if:wiki.history_href ?>
   <li class="last"><a href="<?cs var:wiki.history_href ?>">Historia strony</a></li>
  <?cs else ?>
   <li class="last">Historia strony</li>
  <?cs /if ?>
 </ul>
 <hr />
</div>
...
<div id="content" class="wiki">
 ...
 <?cs if wiki.action == "delete" ?>
  <h1>Usuñ <?cs
   if:?wiki.version ?>wersja <?cs var:wiki.version ?> of <?cs /if ?><a href="<?cs
    var:wiki.current_href ?>"><?cs var:wiki.page_name ?></a></h1>
  <form action="<?cs var:wiki.current_href ?>" method="post">
   <input type="hidden" name="action" value="delete" />
   <p><strong>Czy na pewno chcesz <?cs
    if:!?wiki.version ?>ca³kowicie <?cs /if ?>usun±æ <?cs
    if:?wiki.version ?>wersjê <?cs var:wiki.version ?> <?cs
    /if ?>this page?</strong><br /><?cs
   if:wiki.only_version ?>
    To jest jedyna wersja strony wiêc zostanie ona w ca³o¶ci usniêta!<?cs
   /if ?><?cs
   if:?wiki.version ?>
    <input type="hidden" name="version" value="<?cs var:wiki.version ?>" /><?cs
   /if ?>
   Ta oparacja jest nieodwracalna.</p>
   <div class="buttons">
    <input type="submit" name="cancel" value="Anuluj" />
    <input type="submit" value="Usuñ <?cs
      if:?wiki.version ?>t± wersjê<?cs else ?>strony<?cs /if ?>" />
   </div>
  </form>
 
 <?cs elif:wiki.action == "diff" ?>
  <h1>Zmiany pomiêdzy wersjami <?cs var:wiki.old_version?> oraz <?cs
    var:wiki.version?> <a href="<?cs var:wiki.current_href ?>"><?cs 
  ...
  <form method="post" id="prefs" action="<?cs var:wiki.current_href ?>">
   <div>
    ...
    <label>Zobacz ró¿nice <select name="style">
    ...
    <div class="field">
     ...
     <label for="contextlines">lini dla ka¿dej zmiany</label>
    </div>
    <fieldset id="ignore">
     ...
     <div class="field">
      ...
      <label for="whitespace">Zmiany "white space"</label>
     </div>
    </fieldset>
    <div class="buttons">
  ...
  <dl id="overview">
   ...
   <dd class="author"><?cs
    ...
    /if ?></dd>
   <dt class="time">Kiedy:</dt>
   <dd class="time"><?cs
    if:wiki.num_changes > 1 ?><em class="multi">(multiple changes)</em><?cs
    elif:wiki.time ?><?cs var:wiki.time ?> (<?cs var:wiki.time_delta ?> ago)<?cs
    else ?>--<?cs
    /if ?></dd>
   <dt class="comment">Komentarz:</dt>
   <dd class="comment"><?cs
    if:wiki.num_changes > 1 ?><em class="multi">(wielokrotne zmiany)</em><?cs
  ...
  <div class="diff">
   <div id="legend">
    <h3>Legend:</h3>
    <dl>
     <dt class="unmod"></dt><dd>Niemodyfikowany</dd>
     <dt class="add"></dt><dd>Dodany</dd>
     <dt class="rem"></dt><dd>Usuniêty</dd>
     <dt class="mod"></dt><dd>Zmodyfikowany</dd>
    </dl>
   </div>
   <ul class="entries">
    <li class="entry">
     <h2><?cs var:wiki.page_name ?></h2><?cs
      ...
      <table class="sidebyside" summary="Differences">
       ...
       <thead><tr>
        <th colspan="2">Wersja <?cs var:wiki.old_version ?></th>
        <th colspan="2">Wersja <?cs var:wiki.version ?></th>
       </tr></thead><?cs
