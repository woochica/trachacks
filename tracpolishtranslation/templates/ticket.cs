   ...
   if:len(links.prev) ?>
    <li class="first<?cs if:!len(links.up) && !len(links.next) ?> last<?cs /if ?>">
     &larr; <a href="<?cs var:links.prev.0.href ?>" title="<?cs
       var:links.prev.0.title ?>">Poprzednia metka</a>
    </li><?cs
   /if ?><?cs
   if:len(links.up) ?>
...
<div id="ticket">
 <div class="date">
  <p title="<?cs var:ticket.opened ?>">Otwarto <?cs var:ticket.opened_delta ?> ago</p><?cs
  if:ticket.lastmod ?>
   <p title="<?cs var:ticket.lastmod ?>">Ostatnio modyfikacja <?cs var:ticket.lastmod_delta ?> ago</p>
  <?cs /if ?>
 </div>
 ...
 <table class="properties">
  <tr>
   <th id="h_reporter">Pope³nione przez:</th>
   <td headers="h_reporter"><?cs var:ticket.reporter ?></td>
   <th id="h_owner">Przypisane do:</th>
   <td headers="h_owner"><?cs var:ticket.owner ?><?cs
     if:ticket.status == 'assigned' ?> (zakceptowane)<?cs /if ?></td>
  </tr><tr><?cs
...
<h2>Za³±czniki</h2><?cs
 if:len(ticket.attachments) ?><div id="attachments">
  <dl class="attachments"><?cs each:attachment = ticket.attachments ?>
   <dt><a href="<?cs var:attachment.href ?>" title="Zobacz za³±cznik"><?cs
   var:attachment.filename ?></a> (<?cs var:attachment.size ?>) - dodany przez <em><?cs
 ...
 if:ticket.attach_href ?>
  <form method="get" action="<?cs var:ticket.attach_href ?>"><div>
   ...
   <input type="submit" value="Za³±cz plik" />
  </div></form><?cs
 /if ?><?cs if:len(ticket.attachments) ?></div><?cs /if ?>
<?cs /if ?>

<?cs if:len(ticket.changes) ?><h2>Zmieñ Historiê</h2>
<div id="changelog"><?cs
 each:change = ticket.changes ?>
  ...
  if:len(change.fields) ?>
   ...
   each:field = change.fields ?>
    ...
    if:name(field) == 'attachment' ?><em><?cs var:field.new ?></em> dodano<?cs
    elif:field.old && field.new ?>zmieniono od <em><?cs
     var:field.old ?></em> to <em><?cs var:field.new ?></em><?cs
    elif:!field.old && field.new ?>ustawiono na <em><?cs var:field.new ?></em><?cs
    elif:field.old && !field.new ?>usuniêto<?cs
    else ?>zmieniono<?cs
    /if ?>.</li>
    <?cs
   /each ?>
...
<form action="<?cs var:ticket.href ?>#preview" method="post">
 <hr />
 <h3><a name="edit" onfocus="document.getElementById('comment').focus()">Dodaj/Zmieñ #<?cs
   var:ticket.id ?> (<?cs var:ticket.summary ?>)</a></h3>
 <div class="field">
  <label for="author">Twój adres email lub nazwa u¿ytkownika:</label><br />
  <input type="text" id="author" name="author" size="40"
    value="<?cs var:ticket.reporter_id ?>" /><br />
 </div>
 <div class="field">
  <fieldset class="iefix">
   <label for="comment">Komentarz (mo¿esz tutaj u¿yæ <a tabindex="42" href="<?cs
     var:trac.href.wiki ?>/WikiFormatting">formatowañ Wiki</a>):</label><br />
  ...
  if ticket.comment_preview ?>
   <fieldset id="preview">
    <legend>Podgl±d Komentarz</legend>
 ...
 <?cs if:trac.acl.TICKET_CHGPROP ?><fieldset id="properties">
  <legend>Zmieñ w³a¶ciwo¶ci</legend>
  <table><tr>
   <th><label for="summary">Podsumowanie:</label></th>
 ...
 <fieldset id="action">
  ...
  <?cs call:action_radio('leave') ?>
   <label for="leave">pozostaw jako <?cs var:ticket.status ?></label><br /><?cs
  if:ticket.actions.accept ?><?cs
   call:action_radio('accept') ?>
   <label for="accept">akceptuj metkê</label><br /><?cs
  /if ?><?cs
  if:ticket.actions.reopen ?><?cs
   call:action_radio('reopen') ?>
   <label for="reopen">przypnij ponownie metkê</label><br /><?cs
  /if ?><?cs
  if:ticket.actions.resolve ?><?cs
   call:action_radio('resolve') ?>
   <label for="resolve">zerwij</label><?cs
  ...
  if:ticket.actions.reassign ?><?cs
   ...
   <label for="reassign">przypnij ponownie</label>
   <label>to:<?cs
