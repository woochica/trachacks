...
<div id="ctxtnav" class="nav"><?cs
 if:query.report_href ?><ul>
  <li class="first"><a href="<?cs
    var:query.report_href ?>">Dostêpne raporty</a></li>
  <li class="last">Custom Cytuj</li></ul><?cs
 /if ?>
</div>
...
<form id="query" method="post" action="<?cs var:trac.href.query ?>">
  ...
  <table summary="Query filters">
   ...
   <tbody><tr class="actions">
    <td class="actions" colspan="4" style="text-align: right">
     <label for="add_filter">Ustaw filtr</label>&nbsp;
 ...
 <p class="option">
  ...
  <label for="verbose">Pokazuj pe³ny opis pod ka¿dym z rezultatów</label>
 </p>
 <div class="buttons">
...
<?cs if:len(query.results) ?><?cs
 ...
 each:result = query.results ?><?cs
  ...
  <tr class="<?cs
   ...
   if:result.added ?> dodano<?cs /if ?><?cs
   if:result.changed ?> zmieniono<?cs /if ?><?cs
   if:result.removed ?> usuniêto<?cs /if ?>"><?cs
  each:header = query.headers ?><?cs
   if:name(header) == 0 ?><td class="id"><a href="<?cs
    var:result.href ?>" title="Zobacz ticket"><?cs var:result.id ?></a></td><?cs
...
<div id="help">
 <strong>Pomoc:</strong> Zobacz <a href="<?cs var:$trac.href.wiki ?>/TracQuery">TracQuery</a>, aby dowiedzieæ siê wiêcej o cytowniu.
</div>
