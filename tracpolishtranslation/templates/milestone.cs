...
<div id="content" class="milestone">
  ...
  <form id="edit" action="<?cs var:milestone.href ?>" method="post">
   ...
   <div class="field">
    <fieldset class="iefix">
     <label for="description">Opis (mo¿esz u¿ywaæ <a tabindex="42" href="<?cs
       var:trac.href.wiki ?>/WikiFormatting">Formatowania Wiki</a> here):</label>
   ...
   <div class="buttons">
    <?cs if:milestone.mode == "new"
     ?><input type="submit" value="Dodaj milestone" /><?cs
    else
     ?><input type="submit" value="Zapisz zmiany" /><?cs
    /if ?>
 ...
 <?cs elif:milestone.mode == "delete" ?>
  <form action="<?cs var:milestone.href ?>" method="post">
   ...
   <label for="target">Przekierownie zwi±zanych tickets do milestone</label>
   <select name="target" id="target">
    <option value="">¯aden</option><?cs
 ...
 <?cs if:milestone.mode == "view" ?>
  <div class="info">
   <p class="date"><?cs
    if:milestone.completed_date ?>
     Ukoñczony <?cs var:milestone.completed_delta ?> ago (<?cs var:milestone.completed_date ?>)<?cs
  ...
  <form id="stats" action="" method="get">
   <fieldset>
    <legend>
     <label for="by">Ticket status przez</label>
 ...
 <div id="help">
  <strong>Pomoc:</strong> Zobacz <a href="<?cs
    var:trac.href.wiki ?>/TracRoadmap">TracRoadmap</a>, aby uzyskaæ dodatkowe informacje na temat korzystania z 'map drogowych'.
 </div>
