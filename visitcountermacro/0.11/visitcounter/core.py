    ...
    def expand_macro(self, formatter, name, content):
        if name == 'VisitCounter':
            ...
            if argc >=1:
                ...
                page = formatter.req.path_info[6:] or 'WikiStart'
            if argc >= 2:
            ...
            if update:
                ...
                if formatter.req.session.has_key('visited-pages'):
                    visited = formatter.req.session.get('visited-pages') \
                      .split('|')
                else:
                    visited = []
                ...
                if not (page in visited):
                    ...
                    # Update cookie.
                    visited.append(page)
                    formatter.req.session['visited-pages'] = '|'.join(visited)
                    formatter.req.session.bake_cookie(expires)

            # Add CSS stylesheet.
            add_stylesheet(formatter.req, 'visitcounter/css/visitcounter.css')

            # Set template data and return rendered template.
            db.commit()
            data = {'visitcounter': {}}
            data['visitcounter']['count'] = count
            data['visitcounter']['show_digits'] = digits > 0
            data['visitcounter']['digits'] = self._get_digits(count, digits)
            return to_unicode(Chrome(self.env).render_template(
              formatter.req, 'visitcounter.html', data, 'text/html', True))
