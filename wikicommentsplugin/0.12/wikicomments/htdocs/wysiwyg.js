...
TracWysiwyg.prototype.wikitextToFragment = function(wikitext, contentDocument, options) {
    ...
    var decorationStack;
    var inMacroBlock, inCodeBlock, inParagraph, inDefList, inTable, inTableRow, continueTableRow;
    inMacroBlock = inCodeBlock = inParagraph = inDefList = inTable = inTableRow = continueTableRow = false;

    function handleMacroBlock(line) {
        if (/^ *\{\{\{.*$/.test(line)) {
            inMacroBlock++;
            if (inMacroBlock == 1) {
                closeParagraph();
            ...
            if (inMacroBlock == 0) {
                ...
                macroText.push(line);
                pre.appendChild(contentDocument.createTextNode(macroText.join(
    ...
    for (var indexLines = 0; indexLines < lines.length; indexLines++) {
        ...
        if (inMacroBlock || /^ *\{\{\{.*$/.test(line)) {
            handleMacroBlock(line);
            continue;
        }
    ...
    function open(name, node) {
        ...
        if (token !== undefined) {
            ...
            case "br":
                ...
                break;
            case "pre":
                if ( node.className.indexOf("macro") != -1 ) {
                    _texts.push(
