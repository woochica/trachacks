...
# USA
#
# $Id: stractistics.py 330 2008-02-19 12:13:18Z ddgb $
#
...
class StractisticsModule(Component):
    ...
    def _most_active_repository_authors(self, AUTHORS_LIMIT, start_date, end_date):
        ...
        authors = []
        sql_statement = """
        SELECT r.author author, COUNT( r.author ) commits 
         FROM revision r 
        ...
        authors = [row[0].replace('.',' ') for row in cursor] 
        return authors 
    ...
    def _retrieve_revisions(self, authors, start_date, end_date):
        ...
        revisions = []
        sql_statement = """
        SELECT r.author author, r.time date 
        FROM revision r 
    ...
    def _ticket_activity(self,args):
        ...
        sql_statement = """
        SELECT t.status status, COUNT(DISTINCT t.id) tickets
        FROM ticket t 
    ...
    def _wiki_activity(self):
        ...
        for author in data:
            data[author] = self._values_sorted_by_key(data[author])
    ...
    def _retrieve_most_active_wiki_authors(self, AUTHORS_LIMIT, start_date, end_date):
        ...
        """
        sql_statement = """
        SELECT w.author author, COUNT(distinct w.version) modifications 
        FROM wiki w
        ...
        authors_list = [row[0].replace('.',' ') for row in cursor]
        return authors_list
    
    def _retrieve_wiki_pages(self, start_date, end_date):
        sql_statement = """
        SELECT w.author author, w.time time
        FROM wiki w
